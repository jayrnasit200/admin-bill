...
$(document).ready(function() {
	...
	$('#addCategoriesModalBtn').unbind('click').bind('click', function() {
		...
		$("#submitCategoriesForm").unbind('submit').bind('submit', function() {
			...
			if(billno == "") {
				$("#billno").after('<p class="text-danger">Brand Name field is required</p>');
				$('#billno').closest('.form-group').addClass('has-error');
			} else {
				// remov error text field
				$("#billno").find('.text-danger').remove();
				// success out for form 
				$("#billno").closest('.form-group').addClass('has-success');	  	
			}

			if(bill_date == "") {
				$("#bill_date").after('<p class="text-danger">Brand Name field is required</p>');
				$('#bill_date').closest('.form-group').addClass('has-error');
			} else {
				// remov error text field
				$("#bill_date").find('.text-danger').remove();
				// success out for form 
				$("#bill_date").closest('.form-group').addClass('has-success');	  	
			}

			if(bill_amount == "") {
				$("#bill_amount").after('<p class="text-danger">Brand Name field is required</p>');
				$('#bill_amount').closest('.form-group').addClass('has-error');
			} else {
				// remov error text field
				$("#bill_amount").find('.text-danger').remove();
				// success out for form 
				$("#bill_amount").closest('.form-group').addClass('has-success');	  	
			}
	...
	if(categoriesId) {
		...
		$.ajax({
			...
			success:function(response) {
				...
				$("#edit_billno").val(response.billno);
				// set the categories status
				$("#edit_bill_date").val(response.bill_date);

				$("#edit_bill_amount").val(response.bill_amount);

				$("#bill_gst_121").val(response.gst_no);
